...
당연히 if조건문을 보면 "미입력"만 걸러내니 에러가 발생하게 되었다. 이미 서비스는 배포된 단계라, 충분한 테스트를 거치지 못했던 내 자신의 부족함을 많이 느꼈다. 다행히  블루그린배포로 인해 바로 교체를 할 수 있었지만, 나의 부족함은 틀림없다. 점수는 무조건 숫자이니 내가 여기서 "미입력"아닌 Number와 String타입으로 구별했어야 하는데,  그렇게 생각하지 못했던 것이다. 또한 타입스크립트의 필요성과 도입이 느껴졌다.
